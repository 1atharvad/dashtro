...
import type { SchemaCollectionItem, NewCollectionInput, CollectionsResponse } from '@ts/types/constants';
...
export const fetchCollections = createAsyncThunk("collections/fetchCollections", async (
  ...
  return { projectId, data: await response.json() as CollectionsResponse };
});

export const createCollection = createAsyncThunk("collections/createCollection", async (
  { projectId, newCollection }: { projectId: string; newCollection: NewCollectionInput }
) => {
  const response = await authFetch(`${API_BASE_URL}/projects/${projectId}/collections/`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(
      Object.entries(newCollection).reduce((acc: Record<string, string | number>, [key, value]) => {
...
export const updateCollection = createAsyncThunk("collections/updateCollection", async (
  { projectId, collectionId, updatedCollection }: {
    projectId: string;
    collectionId: string;
    updatedCollection: NewCollectionInput;
  }
) => {
  ...
  return { projectId, data: await response.json() as SchemaCollectionItem };
});
...
type CollectionState = {
  byProject: Record<string, CollectionsResponse>;
  loading: boolean;
  error: string | null;
};
    ...
    builder
      ...
      .addCase(updateCollection.fulfilled, (state, action) => {
        ...
        existing._schema_collections = [
          ...existing._schema_collections.filter((c) => c._id !== data._id),
          data,
      ...
      .addCase(deleteCollection.fulfilled, (state, action) => {
        ...
        const index = existing._schema_collections
          .find((c) => c._id === id)?._index ?? -1;
        existing._schema_collections = existing._schema_collections
          .filter((c) => c._id !== id)
          .map((c) => ({ ...c, _index: c._index > index ? c._index - 1 : c._index }));
      });
  },
});
