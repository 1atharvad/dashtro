...
def create_category(project_id: str, body: dict, request: Request, authorization: str = Header(default=None)):
    ...
    db_audit.log(action='create_category', resource_type='schema_category', user_id=actor['uid'],
                 user_email=actor['email'], resource_id=cat_id, resource_name=name,
...
def update_category(project_id: str, cat_id: str, body: dict, request: Request, authorization: str = Header(default=None)):
    name = (body.get("name") or "").strip()
    if not name:
        raise HTTPException(status_code=400, detail="Category name is required.")
    categories = db.get_categories(project_id)
    ...
    db.upsert_category(project_id, cat_id, {"name": name})
    actor = try_get_actor(authorization)
    db_audit.log(action='update_category', resource_type='schema_category', user_id=actor['uid'],
                 user_email=actor['email'], resource_id=cat_id, resource_name=name,
                 project_id=project_id, ip_address=get_client_ip(request))
    return {"id": cat_id, "name": name}
...
def delete_category(project_id: str, cat_id: str, request: Request, authorization: str = Header(default=None)):
    categories = db.get_categories(project_id)
    if cat_id not in categories:
        raise HTTPException(status_code=404, detail="Category not found.")
    ...
    actor = try_get_actor(authorization)
    db_audit.log(action='delete_category', resource_type='schema_category', user_id=actor['uid'],
                 user_email=actor['email'], resource_id=cat_id, resource_name=cat_name,
                 project_id=project_id, ip_address=get_client_ip(request))


@router.put("/projects/{project_id}/schema-category-map/{schema_name}/")
def set_schema_category(project_id: str, schema_name: str, body: dict, request: Request, authorization: str = Header(default=None)):
    ...
    actor = try_get_actor(authorization)
    db_audit.log(action='assign_category', resource_type='schema_category', user_id=actor['uid'],
                 user_email=actor['email'], resource_name=schema_name,
