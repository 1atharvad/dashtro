        ...
        if not cls._instance:
            ...
            if not firebase_admin._apps:
                ...
                firebase_admin.initialize_app(
                    cred, {"databaseURL": f"https://{project_id}-default-rtdb.firebaseio.com/"}
                )
        return cls._instance

    @staticmethod
    def get_database_name(service_key_path):
        with open(service_key_path) as file:
            data = json.load(file)
...
class FirebaseData(FirebaseClient, Schema):
    def __init__(self):
        ...
        schema = self.store.collection("_schema").get()
        schema_collections = self.store.collection("_schema_collections").get()
