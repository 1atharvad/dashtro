...
class SqliteAuditClient:
    ...
    def _ensure_table(self):
        ...
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_audit_created_at ON cms_audit_logs(created_at DESC);")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_audit_action ON cms_audit_logs(action);")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_audit_resource_type ON cms_audit_logs(resource_type);")
    ...
    def log(
        ...
        cursor.execute(
            """INSERT INTO cms_audit_logs
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (log_id, user_id, user_email, action, resource_type, resource_id, resource_name,
             project_id, workspace_name, details_json, ip_address, created_at)
        )
        self.connection.commit()
        cursor.close()
    ...
    def get_heatmap_data(self, year: int, month: int | None = None) -> list[dict]:
        ...
        if month:
            ...
            for d in range(1, days_in_month + 1):
                ...
                result.append({'date': day_str, 'count': entry['count'], 'top_action': entry['top_action']})
        else:
            import datetime
